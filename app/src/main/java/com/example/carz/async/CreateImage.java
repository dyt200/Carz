...
public class CreateImage extends AsyncTask<CarImage, Void, Void> {
    private AppDatabase database;
    private OnAsyncEventListener callback;
    private Exception exception;

    public CreateImage(Context context, OnAsyncEventListener callback) {
    ...
    protected Void doInBackground(CarImage... params) {
        try {
            for (CarImage carImage : params)
                database.imageDao().insert(carImage);
        } catch (Exception e) {
            exception = e;
        }
        return null;
